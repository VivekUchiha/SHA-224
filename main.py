...
class sha224(object):
    ...
    def update(self, m):
        if not m:
            return
        if type(m) is not str:
            raise TypeError, '%s() argument 1 must be a string, not %s' % (sys._getframe().f_code.co_name, type(m).__name__)
...
z = raw_input("Enter the string to be hashed\n")

print("SHA-224 hash of \"%s\" is : " % (z))

print(sha224(z).hexdigest())